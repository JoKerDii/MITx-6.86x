...
def estep(X: np.ndarray, mixture: GaussianMixture) -> Tuple[np.ndarray, float]:
    ...
    f = (np.sum(X ** 2, axis=1)[:, None] + (delta @ mu.T ** 2) - 2 * (X @ mu.T)) / (
        2 * var
    )  # This is using indicator matrix: fastest of all
    # Pre-exponent term: A matrix of shape (n, K)
    pre_exp = (-np.sum(delta, axis=1).reshape(-1, 1) / 2.0) @ (
        np.log((2 * np.pi * var)).reshape(-1, 1)
    ).T
    # Putting them together gives log of Gaussian joint pdf for a cluster j
    f = pre_exp - f
    ################################ vectorized ended
    ...
    #     f[i, :] = pre_exp - norm / (
    #         2 * var
    #     )  # This is the ith users log gaussian dist vector: (K,)
    ################################ loop ended

    f = f + np.log(pi + 1e-16)  # This is the log of f(u,j), f(u,j) = pi * p(j|u)
...
def mstep(
    ...
    mu_rev[update_indices] = (
        numer[update_indices] / denom[update_indices]
    )  # Only update where necessary (denom>=1)

    # Update variances
    denom_var = np.sum(
        post * np.sum(delta, axis=1).reshape(-1, 1), axis=0
    )  # Shape: (K,)
    ...
    # vectorized version for norm calculation
    # norms = np.sum(((X[:, None, :] - mu_rev)*delta[:, None, :])**2, axis=2)
    norms = (
        np.sum(X ** 2, axis=1)[:, None] + (delta @ mu_rev.T ** 2) - 2 * (X @ mu_rev.T)
    )
