...
def average_perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            ...
            theta_0_sum += theta_0
    return (
        theta_sum / (feature_matrix.shape[0] * T),
        theta_0_sum / (feature_matrix.shape[0] * T),
    )
    raise NotImplementedError
...
def pegasos_single_step_update(
    ...
    if label * ((feature_vector @ current_theta) + current_theta_0) <= 1:
        return (
            (mult * current_theta) + (eta * label * feature_vector),
...
def classify(feature_matrix, theta, theta_0):
    ...
    for i in range(feature_matrix.shape[0]):
        ...
        prediction = (theta @ feature_vector) + theta_0
        if prediction > 0:
            predictions[i] = 1
        else:
            predictions[i] = -1
    ...
    for i, text in enumerate(reviews):
        ...
        for word in word_list:
            if word in dictionary:
                feature_matrix[i, dictionary[word]] = 1  # compute binary features
                # feature_matrix[i, dictionary[word]] += 1  # compute count features
